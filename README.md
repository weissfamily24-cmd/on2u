...
**Status:** Phase 2 — Web-App v0.2 (mobile-first PWA) in `web/`, bereit für den Bamberg-Test
...
├── web/                      ← **Web-App v0.2** — index.html, manifest, sw.js. Das ist die aktuelle Baustelle.
├── app/                      ← Expo-Router-Screens: (tabs)/ Karte·Feed·Scan·Gespeichert·Profil, place/[id]
...
npx expo start
```
Karte und Lokal-Detail laufen mit Demo-Daten. Feed, Scan, Gespeichert, Profil sind Platzhalter — Referenz ist der HTML-Prototyp. Für `react-native-maps` auf Android wird ein Google-Maps-API-Key in `app.json` gebraucht.

## Nächste Schritte

1. Prototyp mit 3–5 Bamberger Gastronomen durchklicken und Feedback sammeln (siehe `docs/concept.md`, Abschnitt „Was wir in Bamberg lernen wollen")
2. Feed-Detail: Kommentare, Folgen, Sound
3. Inhaber-Onboarding: Lokal anlegen, Karte hochladen (Foto → Text), QR drucken
4. Danach: Technik-Entscheidung (React Native + Expo wie die ON2U-App, oder zuerst Web-App)
